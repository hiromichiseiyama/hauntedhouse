...
public class WallRun : MonoBehaviour
{
    ...
    {
        wallRunning = false;
        FPSCamera.cam.rotZ = 0.0f; // Resetting the z rotation on the camera (if the player shoots and the camera shakes it won't be such a problem)
        blocked = true;
    ...
    {
        if (isJumpAvailable && Input.GetKey(KeyManager.Jump))
        {
        ...
        FPSCamera.cam.rotZ = right ? cameraTiltAngle : -cameraTiltAngle;
    }
